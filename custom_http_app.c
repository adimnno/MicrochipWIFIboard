...
/*void HTTPPrint_resultlist(void)
{
 
    for(i = 0; i < resultnum; i++)

    {
        for(x = 0; x < 15; x++)//use the first15 by rssi
        {
            
        
            TCPPutROMString(sktHTTP, (ROM uint8_t*)"<bss>\n");
            TCPPutROMString(sktHTTP, (ROM uint8_t*)"<resultname>");
            TCPPutString(sktHTTP, resulttostore[i].resultssid[x]);
            TCPPutROMString(sktHTTP, (ROM uint8_t*)"</resultname>\n");
            TCPPutROMString(sktHTTP, (ROM uint8_t*)"</bss>\n");
        }
    }
